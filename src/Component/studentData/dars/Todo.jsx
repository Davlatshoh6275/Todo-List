        ...
        return (
            <div className={"container-fluid"}>
                <Modal isOpen={this.state.openModal} toggle={this.openModal}>
                    ...
                    <ModalFooter>
                        <button className={"btn btn-success"} form={"myForm"}>save</button>
                        <button className={"btn btn-warning"} onClick={this.openModal}>cancel</button>
                    </ModalFooter>
                </Modal>
                {
                    this.state.tasks.map(item =>
                        item.isActive ? <Modal isOpen={this.state.openModal2} toggle={this.openModal2}  >
                            <ModalBody>
                            ...
                            <ModalFooter>
                                <button className={"btn btn-success"} form={"myForm2"} >edit</button>
                            </ModalFooter>
                        </Modal> : ""
                    )
                }
                        ...
                        <div className="card">
                            ...
                            <div className="card-body">
                                <ul className={"list-group"}>
                                    {
                                        tasks.filter(tasks => tasks.status === "open").map((tasks, index) => {
                                            return <li className={"list-group-item"} key={index}>
                                                {tasks.title} <span className={"close"}><input value={tasks.title} checked={tasks.isActive} type="checkbox" onChange={this.handleValue} /></span>
                                            </li>
                                        })
                                    }
                                </ul>
                    ...
                    <div className="col-md-3">
                        <div className="card">
                            ...
                            <div className="card-body">
                                <ul className={"list-group"}>
                                    {
                                        tasks.filter(tasks => tasks.status === "pending").map((tasks, index) => {
                                            return <li className={"list-group-item"} key={index}>
                                                {tasks.title} <span className={"close"}><input value={tasks.title} checked={tasks.isActive} type="checkbox" onChange={this.handleValue} /></span>
                                            </li>
                                        })
                    ...
                    <div className="col-md-3">
                        <div className="card">
                            ...
                            <div className="card-body">
                                <ul className={"list-group"}>
                                    {
                                        tasks.filter(tasks => tasks.status === "inprog").map((tasks, index) => {
                                            return <li className={"list-group-item"} key={index}>
                                                {tasks.title} <span className={"close"}> <input value={tasks.title} checked={tasks.isActive} type="checkbox" onChange={this.handleValue} /></span>
                                            </li>
                                        })
